...
module Importer::Efolio
  ...
  def self.import(file)
    ...
    # IO.foreach doesn't read the entire file into memory at once, which is
    # good since a standard FasterCSV.parse on this file can take an hour or
    # more
    lines = []
  ...
  def self.table_name_note
    "notes"
  end

  def self.table_name_note_ts
    "notes_ts"
  end

  def self.store(lines)
    options = {col_sep: ',', row_sep: "\n", skip_blanks: true}
    lines = CSV.parse(lines.join, options)
    return if lines.empty?

    fields_to_sql_fields = fields_to_sql(FIELDS)
    data_to_sql_lines = data_to_sql(lines)

    sql1 = <<-end.squish
      INSERT INTO #{table_name_note} (#{fields_to_sql_fields})
      VALUES #{data_to_sql_lines}
      ON CONFLICT (note_id)
      DO UPDATE SET #{fields_to_update_sql(FIELDS[1..-1])}
    end

    sql2 = <<-end.squish
      INSERT INTO #{table_name_note_ts} (#{fields_to_sql_fields})
      VALUES #{data_to_sql_lines}
    end
